...
│   │   ├── poem.ts        # 诗词相关API
│   │   ├── poet.ts        # 诗人相关API
│   │   ├── poetFeatured.ts # 精选诗人API
│   │   ├── visionArticle.ts # 诗话视野文章API
│   │   ├── forum.ts       # 论坛相关API
│   │   ├── category.ts    # 分类相关API
│   │   ├── dynasty.ts     # 朝代相关API
│   │   ├── file.ts        # 文件上传API
...
│   │   ├── ai.ts          # AI模块API
│   │   ├── admin.ts       # 管理后台API
│   │   ├── admin-ai-model.ts # AI模型配置管理API
│   │   └── external-poetry.ts # 外部诗词API（今日诗词、PoetryDB、接口盒子）
│   └── index.ts           # API统一导出
├── assets/                # 静态资源
│   └── styles/           # 全局样式
...
│       ├── PoetCard.vue
│       ├── HomeNavigation.vue
│       ├── DailyPoetry.vue      # 每日诗词组件
│       ├── PoetrySearch.vue     # 诗词搜索组件
│       └── PoetryDetailDialog.vue # 诗词详情弹窗（接口盒子API增强）
├── composables/           # 组合式函数
│   ├── useAuth.ts        # 认证相关
│   ├── usePoem.ts        # 诗词相关
│   ├── useForum.ts       # 论坛相关
│   └── usePermission.ts  # 权限控制相关
├── layouts/               # 布局组件
│   └── DefaultLayout.vue
├── router/                # 路由配置
...
│   │   ├── dynasties.vue # 朝代管理
│   │   ├── poets.vue    # 诗人管理
│   │   ├── poet-featured.vue # 精选诗人管理
│   │   ├── home-navigation.vue # 首页导航管理
│   │   ├── ai-models.vue # AI模型管理
...
项目使用 `utils/storage.ts` 封装 localStorage 操作，支持过期机制和前缀隔离。

#### 存储键一览

| 存储键 | 说明 | 过期时间 |
|--------|------|----------|
| `moyuan_token` | JWT认证Token | 永不过期 |
| `moyuan_lastAdminUsername` | 上次登录管理员用户名 | 永不过期 |
| `moyuan_user_preferences` | 用户偏好（主题/语言/字体/每页数量） | 24小时 |
| `moyuan_search_history` | 搜索历史（最多20条） | 24小时 |
| `moyuan_form_draft_{type}` | 表单草稿（poem/forum/comment） | 24小时 |
| `moyuan_sort_preferences` | 排序偏好 | 24小时 |
| `moyuan_cache_{key}` | API响应缓存（分类/朝代/诗人） | 1小时 |

#### 使用示例

```typescript
import { getItem, setItem, getCache, setCache } from '@/utils/storage'

// 普通存储（自动带过期机制）
setItem('key', value)

// API缓存（1小时过期）
setCache('dynasties', dynastyList, 60 * 60 * 1000)
const cached = getCache('dynasties')
```
...
项目使用 Vitest 作为单元测试框架，配置文件为 `vitest.config.ts`。
...
```typescript
// src/components/common/LoadingSpinner.test.ts
...
测试文件与源文件同目录，命名规则：`*.test.ts` 或 `*.spec.ts`
...
| 版本 | 日期 | 变更内容 |
|------|------|----------|
| v1.8 | 2026-06-07 | 新增本地存储优化章节，更新项目结构（新增 stores 和 types 文件） |
| v1.7 | 2026-06-06 | 统一响应码类型为 String，更新 Axios 拦截器 code 比较方式 |
| v1.6 | 2026-06-03 | 初始版本 |