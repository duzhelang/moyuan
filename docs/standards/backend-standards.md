...
> **模块说明**：项目当前仅包含一个后端模块：
> - ⚠️ **backend/** — 基础版后端（提供诗词基础CRUD功能，无安全框架、无JWT、无Druid连接池）已合并入 `sc-moyuan-backend/`，目录已不存在，以下结构不再适用
> - **sc-moyuan-backend/** — 完整版后端，包含安全认证、JWT、Druid、Knife4j等完整功能，为当前唯一后端模块
...
### backend/（基础版后端，已合并，不再维护）

> ⚠️ 该模块目录在项目根目录下**已不存在**，功能已全部并入 `sc-moyuan-backend/`。以下结构仅作历史参考。
...
│   │   ├── PoetFeaturedController.java     ✅ 精选诗人控制器
│   │   ├── CategoryController.java         ✅ 分类控制器
│   │   ├── DynastyController.java          ✅ 朝代控制器
│   │   ├── SearchController.java           ✅ 搜索控制器（跨模块搜索）
│   │   ├── FileController.java             ✅ 文件上传控制器
│   │   ├── HistoryController.java          ✅ 浏览历史控制器
│   │   ├── AiController.java               ✅ AI控制器（问答/看图写诗/分析）
│   │   ├── AiModelConfigController.java    ✅ AI模型配置控制器
│   │   ├── HomeNavigationController.java   ✅ 首页导航控制器
│   │   └── AdminController.java            ✅ 管理员控制器
│   ├── service/
│   │   ├── UserService.java                ✅ 用户服务
│   │   ├── PoemService.java                ✅ 诗词服务（含@Cacheable缓存）
│   │   ├── PoetService.java                ✅ 诗人服务（含@Cacheable缓存）
│   │   ├── PoetFeaturedService.java        ✅ 精选诗人服务
│   │   ├── CategoryService.java            ✅ 分类服务
│   │   ├── DynastyService.java             ✅ 朝代服务
│   │   ├── ForumPostService.java           ✅ 论坛帖子服务
│   │   ├── CommentService.java             ✅ 评论服务
│   │   ├── HistoryService.java             ✅ 浏览历史服务
│   │   ├── AiService.java                  ✅ AI服务
│   │   ├── AiModelConfigService.java       ✅ AI模型配置服务
│   │   ├── RecommendationService.java      ✅ 统一推荐服务（诗人+诗词协同过滤）
│   │   └── impl/                           ✅ 服务实现类
│   ├── mapper/                             ✅ Mapper层（14个Mapper）
│   ├── entity/                             ✅ 实体类（14个Entity）
│   ├── dto/                                ✅ 数据传输对象
│   ├── common/
...
> **说明**：后端模块已合并，当前统一使用 `sc-moyuan-backend/` 的 `com.moyuan.common.R<T>` 响应类。原 `backend/` 的 `com.moyuan.common.Result<T>` 仅作历史参考：
> - ⚠️ `backend/` 使用 `com.moyuan.common.Result<T>`（已合并，不再使用）
> - `sc-moyuan-backend/` 使用 `com.moyuan.common.R<T>`（当前唯一响应类）

#### backend/ 中的 Result（已合并，仅作历史参考）
...
> **⚠️ YAML 八进制陷阱（配置规范）**：YAML 中以 `0` 开头的纯数字字符串（如密码 `010125`）会被 SnakeYAML 按八进制解析（`010125` → `4181`），造成数据库 `Access denied` 等诡异认证失败。**密码、账号等以 0 开头的数值型字符串必须加引号**（`password: "010125"`），禁止裸写。建议配置文件中的敏感值一律使用占位符（`${MYSQL_PASSWORD:...}`）或加引号。
...
| 版本 | 日期 | 变更内容 |
|------|------|----------|
| v1.5 | 2026-08-29 | 标注 backend/ 基础版模块已合并入 sc-moyuan-backend/，统一端口为 8085 |
| v1.4 | 2026-06-06 | 统一响应类 R/Result 的 code 字段类型为 String |
| v1.3 | 2026-06-01 | 初始版本 |