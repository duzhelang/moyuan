-- ============================================================
-- 古今诗话——墨渊  补充种子数据脚本（seed.sql）
-- ============================================================
-- 文档：sc-moyuan-backend/src/main/resources/db/init.sql
-- 说明：本脚本用于扩充 poet/poem/poet_featured 表的基础内容，
--       与 init.sql（建表 + 基础种子）分离，独立执行，便于维护。
--
-- 恢复自历史版本（已被清理的补充脚本）：
--   seed-su-shi.sql（苏轼完整数据）
--   insert-foreign-poems.sql（外国诗人 + 外国诗歌）
--   seed-poet-featured.sql（精选诗人卡片）
--
-- 用法：
--   mysql -u root -p moyuan < seed.sql
--   或在 MySQL 客户端中执行本脚本。
--   全部使用 INSERT IGNORE，可重复执行，不会产生重复数据。
--
-- 注意：
--   1. 诗人和诗歌之间存在依赖：必须先插入诗人，再插入诗歌（诗歌用子查询取 poet_id）。
--   2. 本脚本假定 init.sql 已执行（朝代、分类表已有数据）。
--   3. 精选诗人 / 苏轼的配图 image_url 为历史静态资源路径，若前端已改版
--      可能不展示图片，仅影响配图展示，不影响数据正确性。
-- ============================================================
...
-- 一、苏轼完整数据
-- ============================================================
-- poet 表对 (dynasty_id, name) 有唯一索引，使用 INSERT IGNORE 幂等插入。
INSERT IGNORE INTO `poet` (
  `name`,
  `courtesy_name`,
  `pseudonym`,
  `dynasty_id`,
...
北归途中，苏轼于建中靖国元年七月二十八日（1101年8月24日）在常州（今属江苏）逝世。苏轼留下遗嘱葬汝州郏城县（今河南郏县）钧台乡上瑞里。次年，其子苏过遵嘱将父亲灵柩运至郏城县安葬。宋高宗即位后，追赠苏轼为太师；宋孝宗时，追谥"文忠"。',
  '文学
诗作
...
与苏诗一样，苏词中也常常表现对人生的思考。这种对人生命运的理性思考，增强了词境的哲理意蕴。苏轼虽然深切地感到人生如梦，但并未因此而否定人生，而是力求自我超越，始终保持着顽强乐观的信念和超然自适的人生态度。苏词比较完整地表现出作者由积极转而矛盾苦闷，力求超脱自适而不断追求的心路历程和他疏狂浪漫、多情善思的个性气质。继柳永、欧阳修之后，苏轼进一步使词作中的抒情人物形象与创作主体由分离走向同一。
...
王灼：①东坡先生以文章余事作诗，溢而作词曲，高处出神入化，平处尚临镜笑春，不顾侪辈。或曰："长短句中诗也。"为此论者，乃是遭柳永野狐涎之毒。诗与乐府同出，岂当分异？若从柳氏家法，正自不分异耳。②东坡先生非心醉于音律者，偶尔作歌，指出向上一路，新天下耳目，弄笔者始知自振。今少年妄谓东坡移诗律作长短句，十有八九不学柳耆卿则学曹元宠，虽可笑，亦毋用笑也。（《碧鸡漫志·卷二》）
胡寅：词曲者，古乐府之末造也……然文章豪放之士，鲜不寄意于此者，随亦自掩其迹，曰谑浪游戏而已也。唐人为之最工者。柳耆卿后出，掩众制而尽其妙。好之者以为不可复加。及眉山苏氏，一洗绮罗香泽之态，摆脱绸缪宛转之度，使人登高望远，举首高歌，而逸怀浩气，超然乎尘垢之外，于是花间为皂隶，而柳氏为舆台矣。（《〈向芗林酒边集〉后序》）
...
王若虚：①晁无咎云："眉山公之词短于情，盖不更此境耳。"陈后山曰："宋玉不识巫山神女，而能赋之，岂待更而后知？"是直以公为不及于情也。呜呼！风韵如东坡，而谓不及于情，可乎？彼高人逸士，正当如是。其溢为小词，而闲及于脂粉之间，所谓滑稽玩戏，聊复尔尔者也。若乃纤艳淫媟，入人骨髓，如田中行、柳耆卿辈，岂公之雅趣也哉？②公雄文大手，乐府乃其游戏，顾岂于流俗争胜哉？盖其天资不凡，辞气迈故，落笔皆绝尘耳。（《滹南诗话》）
...
-- ============================================================
-- 三、外国诗歌（categoryId 33-41 为外国诗歌分类）
-- ============================================================
